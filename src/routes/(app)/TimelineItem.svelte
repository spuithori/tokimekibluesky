<script lang="ts">
  ...
  import {getPollUrl} from "$lib/components/post/embedUtil";
    ...
    async function muteThread() {
        ...
        try {
            await _agent.xrpcPost('app.bsky.graph.muteThread', {root: getThreadRootUri()});
    ...
    async function unmuteThread() {
        ...
        try {
            await _agent.xrpcPost('app.bsky.graph.unmuteThread', {root: getThreadRootUri()});
    ...
    async function detachQuote(uri: string, unDetach: boolean = false) {
        ...
        try {
            const res = await _agent.xrpcGet('com.atproto.repo.getRecord', {repo: _agent.did(), collection: 'app.bsky.feed.postgate', rkey: rkey});
            detachedEmbeddingUris = res.data?.value?.detachedEmbeddingUris || [];
            embeddingRules = res.data?.value?.embeddingRules || [];
        } catch (e) {
            //
        }
    ...
    async function mute() {
        try {
            await _agent.xrpcPost('app.bsky.graph.muteActor', {actor: data.post.author.did});
    ...
    async function block() {
        try {
            await _agent.xrpcPost('com.atproto.repo.createRecord', {
                repo: _agent.did() as string,
