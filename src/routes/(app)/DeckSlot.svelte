<script lang="ts">
    ...
    const slot = $derived(isJunk ? undefined : columnState.getSlot(index));
    const isSplitLayout = $derived(slot?.layout?.type === 'split');
    const leafIndex = $derived(isJunk ? index : (slot ? columnState.getColumnIndex(firstLeafId(slot.layout)) : -1));
    ...
    function reorderColumns(from: number, to: number) {
        ...
        const popup = slots.map(s => !!columnState.columnById.get(firstLeafId(s.layout))?.settings?.isPopup);
