<script lang="ts">
  ...
  async function getFlatThread() {
    ...
    try {
      const raw = await _agent.xrpc.get('app.bsky.feed.getPostThread', {uri: uri});
      const authorDid = raw.thread?.post?.author?.did;
      const transFormThread = transformThreadView(raw.thread);
