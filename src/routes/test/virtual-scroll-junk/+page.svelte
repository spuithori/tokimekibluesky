<script lang="ts">
  ...
  import VolatileBox from './VolatileBox.svelte';
  ...
  let pendingScheduledChanges = $state(0);
  ...
  onMount(() => {
    ...
    (window as any).__virtualScrollTest = {
      ...
      refreshDividerOnly(count: number) {
        const newItems = generateItems(count, nextId);
        nextId += count;
        const reversed = newItems.reverse();
        reversed[reversed.length - 1].isDivider = true;
        reversed[reversed.length - 1].isHidden = true;
        items = [...reversed, ...items];
      },
      ...
      getItemVisualPosition(testId: string) {
        ...
        return {
          relY: rect.top - containerRect.top,
          height: rect.height,
      ...
      scheduleHeightChanges(changes: Array<{id: string, height: number, delay: number}>) {
        pendingScheduledChanges += changes.length;
        for (const change of changes) {
          setTimeout(() => {
            items = items.map(item =>
              item.id === change.id ? { ...item, height: change.height } : item
            );
            pendingScheduledChanges--;
          }, change.delay);
        }
      },

      getPendingScheduledCount() {
        return pendingScheduledChanges;
      },

      simulateImageLoads(startIndex: number, count: number, extraHeight: number, staggerMs: number) {
        const changes: Array<{id: string, height: number, delay: number}> = [];
        for (let i = 0; i < count && (startIndex + i) < items.length; i++) {
          const item = items[startIndex + i];
          changes.push({ id: item.id, height: item.height + extraHeight, delay: staggerMs * (i + 1) });
        }
