<script lang="ts">
  ...
  let bufferPx = $state<number | undefined>(undefined);
  let useContentHeight = $state(false);
  ...
  onMount(() => {
    ...
    (window as any).__virtualScrollTest = {
      ...
      // Simulate real auto-refresh flow: prepend new items + trim old items
      // This mimics ColumnRefreshButton's: replaceFeed(map) → updateFeed(unshift) → releasePosts(splice)
      prependAndTrim(prependCount: number, maxLength: number) {
        const newItems = generateItems(prependCount, nextId);
        nextId += prependCount;
        // Step 1: map items (simulates replaceFeed with memoryCursor update)
        items = items.map(item => ({ ...item }));
        // Step 2: prepend new items
        items = [...newItems.reverse(), ...items];
        // Step 3: trim to maxLength (simulates releasePosts splice)
      ...
      getDiagnostics() {
        return {
          ...
          topSpacerHeight: virtualList ? Math.max(0, Math.round(virtualList.getLayoutDiagnostics().topGap)) : -1,
      ...
      setTopMargin(value: number) {
        topMargin = value;
      },

      setBufferPx(value: number | undefined) {
        bufferPx = value;
      },

      setUseContentHeight(value: boolean) {
        useContentHeight = value;
      },
      ...
      getBottomSpacerHeight() {
        if (!virtualList) return -1;
        return Math.max(0, Math.round(virtualList.getLayoutDiagnostics().bottomGap));
      },
      ...
      getDebugState() {
        ...
        const diag = virtualList.getTreeDiagnostics();
        const layout = virtualList.getLayoutDiagnostics();
        const renderedItems = scrollContainer?.querySelectorAll('[data-testid^="item-"]');
        let visibleEnd = 0;
        if (renderedItems) {
          for (const el of renderedItems) {
            const idx = parseInt((el as HTMLElement).dataset.index ?? '0', 10);
            if (idx + 1 > visibleEnd) visibleEnd = idx + 1;
          }
        }
        return {
          treeLength: diag.length,
          treeTotal: diag.total,
          bottomSpacerHeight: Math.max(0, Math.round(layout.bottomGap)),
          visibleEnd,
        };
      },
      ...
      getInternalDiagnostics() {
        if (!virtualList) return null;
        const treeDiag = virtualList.getTreeDiagnostics();
        const layout = virtualList.getLayoutDiagnostics();
        return {
          treeTotal: treeDiag.total,
          treeLength: treeDiag.length,
          measuredCount: treeDiag.measuredCount,
          averageHeight: treeDiag.averageHeight,
          topSpacerHeight: Math.max(0, Math.round(layout.topGap)),
          bottomSpacerHeight: Math.max(0, Math.round(layout.bottomGap)),
        };
      },
  ...
  >
    {#snippet children(item: TestItem, index: number)}
      <div
        ...
        data-testid={item.id}
        data-index={index}
        style:height={useContentHeight ? undefined : `${item.height}px`}
        style:min-height={useContentHeight ? `${30 + item.bodyLines * 20}px` : undefined}
