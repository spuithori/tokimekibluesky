...
describe("columnById (id→column の O(1) 解決)", () => {
    it("add/moveLeafToSplit/remove 後も id→column を正しく解決する", () => {
        const { cs, cleanup } = createRealDeckColumnState();
        cs.add(col("a"));
        cs.add(col("b"));

        expect(cs.columnById.get("a")?.id).toBe("a");
        expect(cs.columnById.get("b")?.id).toBe("b");
        expect(cs.columnById.get("ghost")).toBeUndefined();

        cs.moveLeafToSplit("b", "a", "column", false);
        expect(cs.columnById.get("b")?.id).toBe("b");

        cs.remove("a");
        expect(cs.columnById.get("a")).toBeUndefined();
        expect(cs.columnById.get("b")?.id).toBe("b");
        cleanup();
    });

    it("columnById の参照は columns 内の同一オブジェクト(コピーでない)", () => {
        const { cs, cleanup } = createRealDeckColumnState();
        cs.add(col("a"));
        expect(cs.columnById.get("a")).toBe(cs.columns[0]);
        cleanup();
    });

    it("getSlotColumn は columnById と同じオブジェクトを返す", () => {
        const { cs, cleanup } = createRealDeckColumnState();
        cs.add(col("a"));
        cs.add(col("b"));
        cs.moveLeafToSplit("b", "a", "column", false);
        expect(cs.getSlotColumn(0)).toBe(cs.columnById.get("a"));
        cleanup();
    });
});
