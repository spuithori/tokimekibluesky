<script lang="ts">
  ...
  async function mute() {
      isDisabled = true;
      try {
          const mute = await _agent.xrpcPost('app.bsky.graph.muteActor', {actor: user.did});
  ...
  async function unmute() {
      isDisabled = true;
      try {
          const mute = await _agent.xrpcPost('app.bsky.graph.unmuteActor', {actor: user.did});
  ...
  async function block() {
      isDisabled = true;
      try {
          const block = await _agent.xrpcPost('com.atproto.repo.createRecord', {
          ...
          user.viewer.blocking = block.data.uri;
      } catch (e) {
          console.error(e)
      }
      isDisabled = false;
  ...
  async function unblock() {
      ...
      try {
          ...
          const block = await _agent.xrpcPost('com.atproto.repo.deleteRecord', {
              repo: _agent.did() as string,
              collection: 'app.bsky.graph.block',
              rkey: rkey,
          });
