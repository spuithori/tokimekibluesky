...
{#if knownFollowers.followers.length}
  <div class="social-proof">
    ...
    <p class="social-proof-text">
      <button class="social-proof-button" onclick={() => {isModalOpen = true}}>{$_('social_proof_text', {name: knownFollowers.followers[0].displayName || knownFollowers.followers[0].handle, count: knownFollowers.count})}</button>
    </p>
  </div>
{/if}
