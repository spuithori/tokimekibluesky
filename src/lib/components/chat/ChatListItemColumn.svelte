<script lang="ts">
    ...
    async function leaveChat() {
        try {
            const res = await currentAgent.xrpcPost('chat.bsky.convo.leaveConvo', {convoId: convo.id}, { proxy: CHAT_PROXY });
    ...
    async function muteChat() {
        try {
            const res = await currentAgent.xrpcPost('chat.bsky.convo.muteConvo', {convoId: convo.id}, { proxy: CHAT_PROXY });
    ...
    async function unMuteChat() {
        try {
            const res = await currentAgent.xrpcPost('chat.bsky.convo.unmuteConvo', {convoId: convo.id}, { proxy: CHAT_PROXY });
            const id = res.data.convo.id;
