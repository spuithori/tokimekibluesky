<script lang="ts">
    ...
    import { buildIndex, searchSettings, categoryLabel } from '$lib/settings/search.svelte';
    ...
    function navigate(route: string, key: string) {
        query = '';
        goto(`${route}#${encodeURIComponent(key)}`);
    }
    ...
    function handleKeydown(event: KeyboardEvent) {
        ...
        if (event.key === 'ArrowDown') {
            ...
            if (result) {
                navigate(result.route, result.key);
            }
...
<div class="settings-search">
    <Search class="settings-search__icon" size="18" color="var(--text-color-3)"></Search>
    <input
    ...
    <ul class="settings-search__results">
        {#each results as result, i (result.key)}
            <li class="settings-search__result">
                <a
                    href="{result.route}#{encodeURIComponent(result.key)}"
                    class:settings-search__result-link--selected={i === selectedIndex}
                    class="settings-search__result-link"
                    onclick={(event) => { event.preventDefault(); navigate(result.route, result.key); }}
                >
                    <span class="settings-search__result-label">{result.label}</span>
                    <span class="settings-search__result-cat">{$_(categoryLabel[result.category])}</span>
                </a>
            </li>
        {:else}
            <li class="settings-search__empty">{$_('settings_search_no_results')}</li>
        {/each}
    </ul>
{/if}
