...
<section class="feed feed--{layout}">
    ...
    <div class="feed__content">
      ...
      <div class="feed__buttons">
        ...
        {#if (layout === 'default' || layout === 'embed')}
          ...
          <a href="/profile/{feed.creator.did}/feed/{feed.uri.split('/').slice(-1)[0]}" onclick={setCurrentFeed} class="button button--border button--ss">{m.feed_show_button()}</a>
        {/if}
      </div>
    </div>
  </div>
  ...
  {#if (layout !== 'embed' && layout !== 'publish')}
    ...
    </svg>{m.feeds_toggle_creator()}</button>
