        ...
        <p class="label-detail__user">
          ...
          {:else}
            ...
            {#if (label?.label?.src === 'did:plc:ar7c4by46qjdydhdevvrndac')}
              <a href="/profile/did:plc:ar7c4by46qjdydhdevvrndac" onclick={onclose}>{$_('labeling_source_labeler_official')}</a>
            {:else}
              <a href="/profile/{label?.label?.src}" onclick={onclose}>{$_('labeling_source_labeler')}</a>
            {/if}
          {/if}
        </p>
